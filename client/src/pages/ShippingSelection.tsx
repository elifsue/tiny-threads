      ...
      <div className="px-6 py-6 flex-1">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mt-5 border border-gray-200 rounded overflow-hidden mb-5">
            ...
            {isHifi ? (
              <img
                src="https://d2xsxph8kpxj0f.cloudfront.net/310519663332337268/5dAnQahEr3KsPSbq9QaW5W/london-map-pickup-points-3YQP8tJpkeJ9YsxUE3ZcQ6.webp"
                alt="Map showing nearby pick-up points in London"
                className="h-72 w-full object-cover"
              />
            ) : (
              <ImgPlaceholder label="Map — Pick-up Points" className="h-72" />
            )}
            <div className="p-3 flex flex-col gap-2">
              {[
                { name: "InPost Locker — Tesco Express, 0.3 mi", open: true },
                { name: "Evri ParcelShop — Corner News, 0.5 mi", open: true },
                { name: "InPost Locker — Sainsbury's Local, 0.8 mi", open: false },
              ].map((loc) => (
                <div key={loc.name} className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full flex-shrink-0" style={isHifi ? { backgroundColor: loc.open ? '#22c55e' : '#ef4444' } : { backgroundColor: '#d1d5db' }} />
                  <span className={`text-xs ${isHifi ? "text-gray-700" : "text-gray-500"}`}>{loc.name}</span>
                </div>
              ))}
            </div>
          </div>
